...
foreach ($mailbox in $sharedMailboxes) {
    try {
        $owners = Get-MailboxPermission $mailbox.Identity | Where-Object { $_.IsInherited -eq $false -and $_.AccessRights -like "*FullAccess*" } | Select-Object User
...
# Exporting to CSV
$results | Export-Csv -Path "Shared-Mailbox-Data-FC.csv" -NoTypeInformation
